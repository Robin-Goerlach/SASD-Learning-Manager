...
/// <summary>Lists and edits professional contacts as first-class records.</summary>
public sealed class ContactsControl : UserControl
{
    ...
    private IReadOnlyList<Contact> _items = [];

    /// <summary>Initializes the contacts view.</summary>
    public ContactsControl(ContactService service, OrganizationService organizations, UiExceptionPresenter errors)
    {
        _service = service;
    ...
    private async Task RefreshAsync()
    {
        try
        {
            ...
            _grid.DataSource = _items.Select(item => new
            {
                item.Id,
                Name = item.FullName,
                Organisation = item.OrganizationId is Guid id && names.TryGetValue(id, out var name) ? name : string.Empty,
                Rolle = item.Role ?? string.Empty,
                EMail = item.Email ?? string.Empty,
                Telefon = item.Phone ?? string.Empty,
            }).ToList();
    ...
    private async Task CreateAsync()
    {
        try
        {
            ...
            using var dialog = new ContactEditForm(orgs);
    ...
    private async Task EditSelectedAsync()
    {
        ...
        try
        {
            // Keep an already archived organization selectable while editing historical contacts.
            var orgs = await _organizations.ListAsync(includeArchived: true);
            using var dialog = new ContactEditForm(orgs, selected);
            if (dialog.ShowDialog(this) != DialogResult.OK)
            {
                return;
            }
